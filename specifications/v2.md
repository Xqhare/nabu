...
It is best explained as a binary JSON variation.

`.xff` stands for `xqhares file format` or `xqhared file format`, pronounced `squares file format` or `squared file format`.

Version 2 is a small extension of version 1. 
A version 2 Parser can be constructed in a way to parse version 1 as well.

The public API has not changed in any way from version 1.

While this version of the `.xff` specification is designed to be compatible with version 1, several changes have been made that require special attention for implementing a parser for both version 1 and 2. These can be found in the way the length is encoded, as well as the addition of a checksum.
...
In it, you can see two coloured regions, one in a light blue in the center, and on in light green surrounding it. \
...
> [!important]
> Any implementation of the `.xff` specification should be able to read and write all versions of the specification, but may choose to support any set of versions or single version and to implement either the ability to read or write, leaving out the other.
...
The trailing `EV` provides an additional check for correctness, as well as delimiting the value.
...
## Numbers
A valid `Number` is a sequence of decimal digits (`0` to `9`) with no superfluous leading zero, containing a single `.` or `,` decimal separator between digits.
...
Command characters cannot be used.

`Number` values may not be empty.

This specification allows implementations to set arbitrary limits on the range and precision of numbers accepted, but it recommends the implementation of IEEE754.
...
All data inside a `Data` value is considered one unified data-stream that is stored as-is and cannot not be changed in any way.
