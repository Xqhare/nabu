...
> [!important]
> Any implementation of this specification should be able to read and check the correctness of the data with the checksum of the entire file, as well as the checksum of single values.
> It must be able to generate and write the checksum of the entire file, as well as the checksum of single values.
...
Strings have to be encoded in [ASCII](xff-byte-encoding.md) and character codes 8 through 13 are permissible.

`TXT` is the start of every `String`.
...
The trailing `EV` provides an additional check for the correctness of the value, as well as delimiting the value.

The light blue coloured region in the diagram below shows the range of calculation for the value checksum.
...
A number is opened with `NUM`, followed by the length of the number in bytes, followed by the number itself. 
Behind the number is the `CHK` marker and the checksum of the number.
The number is closed by a `EV` byte.
...
All data inside on `Data` value is considered one unified data-stream that is stored as-is and may not be changed in any way.

Data is opened with `DAT`, followed by the length of the data in bytes, followed by the data itself.
Behind the data is the `CHK` marker and the checksum of the data.
The data is closed by a `EV` byte.
...
To open an array, `ARY` is used, followed by the length of the array in bytes, followed by the array itself. 
...
To open an object, `OBJ` is used, followed by the length of the object in bytes, followed by the object itself. 
Behind the object is the `CHK` marker and the checksum of the object.
It is closed by `EV`.

The trailing `EV` provides an additional check for the correctness of the value, as well as delimiting the value.

Key-value pairs are enclosed by `GS`, the key and the value are separated by `US`.
