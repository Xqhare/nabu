...
1.  **`[Column Schema]`**: `[Column Count (LEB128)]` + `[Column Offsets]` + a sequence of `Column Count` `Text` values representing column names. `Column Offsets` is a sequence of LEB128 offsets, each specifying the start of a column name relative to the start of the first column name.
2.  **`[Row Index]`**: `[Row Count (LEB128)]` + `[Row Offsets]` + `[Element Offsets]`. 
    *   `[Row Offsets]`: A sequence of `Row Count` LEB128 offsets pointing to the start of each row.
