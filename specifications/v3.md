...
<details>
    <summary>Detailed descriptive summary for visually impaired users (Inlined)</summary>

This description is organized logically to assist visually impaired users in understanding the structural flow and data validation rules of the format.

### 1. High-Level Overview

The diagram illustrates a linear, left-to-right flow of a binary file. It is divided into three primary sections: a **Global Header**, a **HEAD** block (for metadata), and a **BODY** block (for data payloads). The format uses a strict validation pattern where data segments are followed by a **CHECKSUM** and an **EV** (End Value) marker.  
**Validation Logic Note:** Throughout the diagram, specific boxes have a **faint green glow**. This glow indicates the exact boundary of data that is being hashed or verified. The glow always appears on the data box immediately to the left of a **CHECKSUM**, signifying that the checksum validates that specific "glowing" content.

### 2. Global Header & HEAD Section

The file begins with two mandatory entry points:

1.  **File Signature:** The literal string `xffV`.
2.  **Version:** A field defining the format version.

Following these is the **HEAD** container, which stores metadata. It follows this sequence:

*   **META Marker:** Marks the start of metadata.
*   **Element Count & Offsets:** Defines the number of metadata entries and their positions.
*   **The Payload (Glowing):** A repeating loop of KEY (Text) and VALUE.
*   **Validation:** A **CHECKSUM** (validating the Key/Value loop) followed by an **EV** marker.

### 3. The BODY Section

The **BODY** is the largest container. It consists of a vertical list of optional data types. A user parsing the file would identify the type and then follow the specific structure defined for that type.

#### Simple Fixed & Variable Length Types

These types follow a standard pattern: **[Type Name] → [Data Content] → [Checksum] → [EV]**.

| Data Type | Internal Content (The "Glowing" Data) |
| :--- | :--- |
| **DATETIME / DURATION** | Encoded as **LEB-128** (variable-length integer) bytes. |
| **UUID** | A fixed **16-byte** raw binary string. |
| **NUM UINT / SINT** | Unsigned or Signed integers encoded via **LEB-128**. |
| **NUM FLOAT** | A raw **64-bit (f64)** floating-point number. |
| **TEXT** | A **Length** prefix followed by the **UTF-8 bytes**. |
| **DATA** | A **Length** prefix followed by **Raw binary bytes**. |

#### Complex & Nested Types

These types involve multiple elements or structural metadata before the actual values:

*   **ARRAY:** Contains **Element Count** and **Element Offsets**, followed by a glowing **VALUE** block (repeated for each element), a **CHECKSUM**, and an **EV**.
*   **OBJECT / ORDERED OBJECT:** Contains **Element Count** and **Element Offsets**, followed by a glowing loop of **KEY(Text)** and **VALUE** pairs, a **CHECKSUM**, and an **EV**.
*   **TABLE:** The most complex structure. It includes **Column Count**, **Column Offsets**, and **KEYs**, followed by **Row Count** and **Row Offsets**. The actual data is a glowing block of **VALUE**s, followed by a **CHECKSUM** and an **EV**.

#### Constant Values (Literals)

At the bottom of the BODY, there is a list of constant values that do not require lengths or checksums:

*   **Booleans:** TRUE, FALSE.
*   **Floats:** INFINITY, NEGATIVE INFINITY, NOT A NUMBER.
*   **Nulls:** NULL.

### 4. Termination

After the BODY section concludes, all logical paths converge at a single final block:

*   **EM:** This stands for **End Marker**, signifying the physical end of the xff_v3 file.
...
**Structure**: `[META Marker] [Element Count] [Offsets...] [Key/Value Data...] [Checksum] [EV]`
