...
The byte-structure of `float` and `integer` are similar enough that there are byte-sequences that are shared.
...
However, the idea has merit, and the easiest way I see is to push it to v4 and introduce it with a new `Number` version.
...
After some fiddeling, I have found lzw leb encoded to compress most files (there are still cases of size inflation) - Still takes 40sec for 100mb.

NAK
After considering all of the above, this isn't suitable for the spec. Also, if there ever is a need for compression, they can (and probably will anyway) just zip it.
