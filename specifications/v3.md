...
3.  **`[Checksum]`**: A 4-byte CRC-32 covering the `[Element Count]` and `[Offsets]`.
4.  **`[Data Block...]`**: The contiguous block containing the serialized child values.
...
1.  **`[Column Index]`**: `[Column Count (LEB128)]` + `[Column Offsets]` + `[Checksum]`.
    ...
    *   `[Checksum]`: A 4-byte CRC-32 covering the `[Column Count]` and `[Column Offsets]`.
...
3.  **`[Row Index]`**: `[Row Count (LEB128)]` + `[Row Offsets]` + `[Checksum]`.
    ...
    *   `[Checksum]`: A 4-byte CRC-32 covering the `[Row Count]` and `[Row Offsets]`.
4.  **`[Element Index]`**: `[Element Offsets]` + `[Checksum]`.
    ...
    *   `[Checksum]`: A 4-byte CRC-32 covering the `[Element Offsets]`.
5.  **`[Row Data]`**: The contiguous block of row values. Each row consists of exactly `Column Count` values, matching the order and types implied by the schema. Rows are not individually marked as `Array` or `Object` to save space.
