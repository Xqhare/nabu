...
In contrast to version 2, the length field is no longer mandatory for all values. In version 3 it is used only for types where the content size is not inherently fixed or self-terminating, such as `Text`, `Data`, or `Parent` types.

![Chart of the composition of values in a `.xff` file in token form.](../pictures/xff_v3-complete-chart.png)

> [!note]
> **Validation Logic:** Throughout the diagram above, specific boxes have a **faint green glow**. This glow indicates the exact boundary of data that is being hashed or verified. The glow always appears on the data box immediately to the left of a **CHECKSUM**, signifying that the checksum validates that specific "glowing" content.
>
> The Glow may appear on one box, or several (though the spec does not need more than two boxes, this is not a hard rule, but because of happenstance (Structure of the values)).
>
> Examples:
> - `[NUM FLOAT]` `[8-byte raw f64]` `[CHECKSUM]` `[EV]` - ONLY `[8-byte raw f64]` glows
> - `[TEXT]` `[Length]` `[UTF-8 bytes]` `[CHECKSUM]` `[EV]` - BOTH `[Length]` and `[UTF-8 bytes]` glow
...
3.  **Standard Keys**: Implementations are encouraged to use standard keys: 
    ...
    * `license`: The legal license governing the data. The `license` key may be used to store an abbreviated license string or the entire license.
        ...
        * Example: `license` = `"Copyright (c) 2024 - 2026 Xqhare Permission is hereby granted, free of charge, ..."`
...
| 160 |`0xA0` | 10100000 | TXT | Text (UTF-8) | Complex |
| 33 | `0x21` | 00100001 | DAT | Data (Binary) | Complex |
| 34 | `0x22` | 00100010 | SINT | Signed Integer (LEB128) | Complex |
| 163 | `0xA3` | 10100011 | FLT | Float (f64) | Complex |
| 36 | `0x24` | 00100100 | UINT | Unsigned Integer (LEB128) | Complex |
| 165 | `0xA5` | 10100101 | DUR | Duration | Complex |
| 166 | `0xA6` | 10100110 | UUID | UUID | Complex |
| 39 | `0x27` | 00100111 | DT | Date and Time | Complex |
...
*   `NaN` / `Infinity` / `NegInfinity`: Floating-point special values.
    *   **Note**: These can also be stored as a `Float` value (which is explicitly valid XFF). However, storing as such will require 14 bytes (Marker + CRC-32 + 8-byte f64 + EV), whereas using these Simple Value markers reduces it to exactly **1 byte**.
    *   **Best practice**: If `NaN`, etc., are rare edge cases and normal output is expected as a float, use the `Float` type. If `NaN`, etc., are frequently expected as output, use these Simple Value markers to save on space and encoding/decoding overhead.
