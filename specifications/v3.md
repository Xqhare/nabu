...
1.  **Constraints**: To simplify parsing of the file head, the metadata object must be **flat**. It may contain parent values, but they cannot be nested.
...
3.  **Standard Keys**: Implementations are encouraged to use standard keys: 
    ...
    * `license`: The legal license governing the data. The `license` key may be used to store an abbreviated license string or the entire license.
        ...
        * Example: `license` = `"Copyright (c) 20XX Copyright Holder. Permission is hereby granted, free of charge, ..."`
