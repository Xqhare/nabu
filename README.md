...
Any mention of ASCII should be understood to be referring to Windows-1252 specifically. [See the reference table here.](#windows-1252-reference-table)
...
Any implementation of `.xff` should be able to read any version of the `.xff` specification, but may choose to only support a specific set of versions or version.
...
The difference between `String` and `Number` is that a valid `Number` is any valid number as according to the 2nd Edition of `ECMA-404` from December 2017, and any different content is a `String`.
...
Directly after the opening `DLE` are 5 bytes (36 bits) that indicate the length of the data in bytes, and then the data itself.
...
This limit was chosen arbitrarily, and could be increased if necessary, however 8 bytes would encode 18.4 million terabytes of data but waste the most space.
With 4 bytes, encoding 4.2 gigabytes of data should also be enough already, in my most conservative estimate even 2 to 3 bytes should be enough.
I do not expect anything to use anywhere near the full terabyte, especially with current hardware.
On the other hand a few wasted bytes should not hurt on modern hardware also, and this does give support to put the entirety of GTA-V or a current CoD into one escape.
