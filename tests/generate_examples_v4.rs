...
fn generate_v4_examples() {
    ...
    write_legacy(
        format!("{}v4_boolean_f.xff", base),
        XffValue::from(false),
        4,
    )
    .unwrap();
    write_legacy(format!("{}v4_nan.xff", base), XffValue::NaN, 4).unwrap();
    write_legacy(format!("{}v4_pnan.xff", base), XffValue::PosNaN, 4).unwrap();
    write_legacy(format!("{}v4_nnan.xff", base), XffValue::NegNaN, 4).unwrap();
    write_legacy(format!("{}v4_infinity.xff", base), XffValue::Infinity, 4).unwrap();
    write_legacy(
        format!("{}v4_neginfinity.xff", base),
    ...
    // New complex types
    write_legacy(
        format!("{}v4_local_date.xff", base),
        XffValue::LocalDate(LocalDate::new(2026, 5, 16)),
        4,
    )
    .unwrap();
    write_legacy(
        format!("{}v4_local_time.xff", base),
        XffValue::LocalTime(LocalTime::new(14, 30, 0, 0)),
        4,
    )
    .unwrap();
    write_legacy(
        format!("{}v4_local_datetime.xff", base),
    ...
    .unwrap();
    write_legacy(
        format!("{}v4_hp_float.xff", base),
        XffValue::HpFloat(HpFloat::new(123456789, 4)),
        4,
    )
    .unwrap();
    write_legacy(
        format!("{}v4_ascii.xff", base),
        XffValue::Ascii(athena::XffString::from("ASCII text")),
        4,
    )
    .unwrap();
    ...
    g.add_connection(n1, n2, XffValue::from("connection"))
        .unwrap();
