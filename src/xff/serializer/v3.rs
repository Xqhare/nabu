...
pub fn serialize_xff_v3(data: Vec<XffValue>) -> Result<Vec<u8>> {
    ...
    if let Some(XffValue::Metadata(meta)) = data.first() {
        ...
        serialize_xff_v3_with_metadata(vec![body], Some(meta.clone().map))
    } else {
...
fn serialize_v3_value(value: &XffValue) -> Result<Vec<u8>> {
    match value {
        ...
        XffValue::Number(n) => serialize_v3_number(n),
...
fn serialize_v3_number(n: &athena::Number) -> Result<Vec<u8>> {
    if n.is_float() {
        ...
        if val.is_nan() {
            Ok(vec![ensure_even_parity(NAN)])
        } else if val.is_infinite() {
            if val.is_sign_positive() {
                Ok(vec![ensure_even_parity(INF)])
            } else {
                Ok(vec![ensure_even_parity(NINF)])
            }
        } else {
